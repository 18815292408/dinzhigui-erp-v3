...
SELECT '007: confirmed designs' as migration, 
       CASE WHEN COUNT(*) = 0 THEN '✅ 已完成' ELSE '❌ 未完成' END as status
FROM designs WHERE status = 'confirmed';

-- 2. 验证 users 表是否有 role_limits 字段 (008)
SELECT '008: users.role_limits' as migration,
       CASE WHEN COUNT(*) = 1 THEN '✅ 已完成' ELSE '❌ 未完成' END as status
FROM information_schema.columns 
WHERE table_name = 'users' AND column_name = 'role_limits';

-- 3. 验证 users 表是否有 can_manage_users 字段 (009)
SELECT '009: users.can_manage_users' as migration,
       CASE WHEN COUNT(*) = 1 THEN '✅ 已完成' ELSE '❌ 未完成' END as status
FROM information_schema.columns 
WHERE table_name = 'users' AND column_name = 'can_manage_users';

-- 4. 验证 installations.feedback 是否为 JSONB 类型 (010)
SELECT '010: feedback is JSONB' as migration,
       CASE WHEN COUNT(*) = 1 THEN '✅ 已完成' ELSE '❌ 未完成' END as status
FROM information_schema.columns 
